...
describe('9 - Implemente os casos de teste da função `getCharacter`', () => {
  it('Verifica se a função `getCharacter` retorna o objeto do personagem corretamente.', () => {
    ...
    const expected = {name: 'Arya Stark', class: 'Rogue', phrases: [ 'Not today', 'A girl has no name.' ]};
    expect(getCharacter('ARYA')).toEqual(expect.objectContaining(expected));
    expect(getCharacter('arya')).toEqual(expected);
    // Teste se ao passar um nome que não está na tabela, a função retorna undefined.
    expect(getCharacter('Michel')).toBeUndefined();
  });
});
