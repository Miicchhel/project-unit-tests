...
const orderFromMenu = (obj, string) => obj.consumption.push(string);

const createMenu = (objeto) => {
  const menu = {
    consumption: [],
    fetchMenu: () => objeto,
    order: (string) => { orderFromMenu(menu, string); },
    pay: () => {
      let soma = 0;
      const arrayKeys = Object.keys(objeto);
      for (let elem of menu.consumption) {
        let [valor1, valor2] = [objeto[arrayKeys[0]][elem], objeto[arrayKeys[1]][elem]];
